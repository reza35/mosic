...
;--------------------------------------------------------------------------
  if (ergs.w1 lt 1.0d)or(ergs.w1 gt 14.0/dfac) then ergs.w1 = 8.0/dfac
  if (ergs.i1 gt tmax*1.1) then ergs.i1 = tmax
  if (abs(ergs.p1 - posmax) gt 9.) then ergs.p1 = posmax

  fit0 = [ergs.i1>4., ergs.p1, ergs.w1 > .2]
  fit1 = [max(fitsg)*0.82d, ergs.p1 - 88./dfac] 
  ...
  if (ergd.i1/ergd.i2 gt 1.5)or(ergd.i1/ergd.i2 lt 0.9)or(n_elements(fitdg) lt np)or(chisq2 gt 50.) then begin
     ; we repeat the fit with amplitude of the second line fixed with  a ratio
  
     fit2 = [ergs.i1>1., ergs.p1, ergs.w1, fit1[1]]
...
ee =  ir_error(py, /fuv, /dark)
  ...
  v_peak = max(py[(ergd.p1 - 15/dfac):(ergd.p1 -3/dfac)], v_posmax) & v_posmax = (v_posmax + (ergd.p1 - 15/dfac))> (ergd.p1 - disper*3.) 
  r_peak = max(py[(ergd.p1 +3/dfac):(ergd.p1 + 15/dfac)], r_posmax) & r_posmax = (r_posmax + (ergd.p1 +3/dfac)) > (ergd.p1) < (np -5.)

  b_peak = max(py[((ergd.p2 - 15/dfac)>0):(ergd.p2 -3/dfac)], b_posmax) & b_posmax = (b_posmax + (ergd.p2 - 15/dfac))> (ergd.p2 - disper*3.) 
  g_peak = max(py[(ergd.p2 +3/dfac):(ergd.p2 + 15/dfac)], g_posmax) & g_posmax = (g_posmax + (ergd.p2 +3/dfac)) > (ergd.p2) < (np -5.)
 
  fit0 = [ergd.i1*0.8 > 2., v_posmax, ergd.w1, ergd.i2*0.8>2., b_posmax]
  fit1 = [ergd.i1*0.8d> 2., r_posmax, ergd.w1, ergd.i2*0.8>2., g_posmax] 
  ...
  fit0 = [ergd.i1 > 2., ergd.p1, ergd.w1, ergd.i2>2., ergd.p2]  ; first Gaussian, both lines
  
  ;if (disper lt 2.) then begin
  fit1 = [ergd.i1*0.3d, ergd.p1 - 3.0/dfac, ergd.w1 * 1.2, ergd.i2*0.3d, ergd.p2 - 3.0/dfac] 
  ;endif else begin
  ;   fit1 = [ergd.i1*0.3d, ergd.p1 - 1.5, ergd.w1 * 1.2, ergd.i2*0.3d, ergd.p2 - 1.5] 
  ;endelse   
  ...
  if (n_elements(fitqg) lt np)or(chisq10 gt 50.) then begin ; something went wrong
                                 ; we repeat the fit with amplitude of the second line fixed with  a ratio
     fit2 = [ergs.i1>1., ergs.p1, ergs.w1, fit1[1]] * 1.0d
...
if (fast eq 0) then begin  
  ...
  ;if (disper lt 2.) then begin
  fit1 = [max(fitsg)*0.1d > 2., ergs.p1 - 38./dfac, (ergd.w1 * 0.4)>.2]   ; Ni II parameters
  ;endif else begin
  ;   fit1 = [max(fitsg)*0.1d > 2., ergs.p1 - 19., (ergd.w1 * 0.4)>.2]
  ;endelse   
  ...
  dlambda = 1.0d
  pyn = ppy - ergs.b
  ;ee[((ergs.p1 - 38/dfac)>0):(ergs.p1 - 20/dfac)]=100.
  ;print, fit0, fit1
